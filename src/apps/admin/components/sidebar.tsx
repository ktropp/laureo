...
export function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  return (
    <div className={`
      fixed left-0 top-0 h-full bg-card border-r border-slate-300 dark:border-slate-600 transition-all duration-300 z-50
      ${collapsed ? 'w-16' : 'w-64'}
    `}>
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-slate-300 dark:border-slate-600">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-lg">Laureo CMS</span>
          </div>
        )}
        <Button
          variant="ghost"
