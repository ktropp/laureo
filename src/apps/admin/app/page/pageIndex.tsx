...
export default function PagesIndex({ initialData }) {
    const [open, setOpen] = useState(false);
    const [postToDelete, setPostToDelete] = useState(null);

    const handleDelete = async () => {
        if (postToDelete) {
            //await pageDelete(pageToDelete);
            setOpen(false);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <Button asChild><Link href="/page/add">Add <CirclePlus className="h-5 w-5"/></Link></Button>
            </div>
